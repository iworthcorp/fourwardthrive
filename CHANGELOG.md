...
### Added
- **00:50** — Excluded the internal brand-guideline reference PDF (`FOURWARD THRIVE BRANDING.pdf`) from version control via `.gitignore`.
...
### Changed
- **00:37** — Replaced the site's gold/amber accent color with the brand-approved palette from `FOURWARD THRIVE BRANDING.pdf`: Primary Blue (`#007BFF`) for buttons, headings, links, and active nav state; Navy (`#0A1F44`) / White (`#FFFFFF`) for backgrounds and text; Silver Gray (`#B8C0CC`) for borders and dividers — in both light and dark mode. Since every component already referenced these as semantic Tailwind tokens (`noir-primary`, `noir-bg`, etc.) rather than hardcoded colors, this was a single-file change to the `--noir-*` custom properties in `app/globals.css`.
...
### Fixed
- **00:30** — Fixed a production build failure (`Type error: Could not find a declaration file for module 'react-dom'`) by adding the missing `@types/react-dom` dev dependency — needed for `LightboxGallery.tsx`'s `createPortal` import from `react-dom`, which had types for `react` but not `react-dom` installed.
